...
(block outer
	...
	; Collect protos as expanded, shared-scope children
	; of the compo
		...
		if (and
			...
			do (let ((bind-pair (expand-nested-schema s composite-schema)))
				(setf concrete-protos (append concrete-protos (list
					(apply-bindings (car bind-pair) (second bind-pair)))))
			))

	; Print the story, but if any of the episode formulas 
	; are characterized by a protoschema header, use the
	; header formula instead of the episode.
	(loop for phi in el-story do (block pr
		(setf got-proto nil)
		(if (and (canon-charstar? phi)
			(loop for cp in concrete-protos
				do (setf got-proto cp)
				thereis (equal (third phi) (third (schema-header cp)))))
			; then
			(block rewrite-phi
				; (format t "rewriting phi~%")
				(setf header (schema-header got-proto))
				(setf header-phi (car header))
				(setf header-ep (third header))
				(setf header-pred (prop-pred header-phi))
				; update the header predicate name
				; to encode which protoschema it
				; came from
				...
				; print the new header instead
				; of the story formula
				(format t "~s~%" (list
					header-phi
					'**
					header-ep))
