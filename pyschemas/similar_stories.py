...
def make_topical_stories(topic, rep_pen=1.1, temp=0.3, resp_length=128, howto=False, starting_word=''):
	prompt = TOPIC_PROMPT % (topic.strip())
	if len(starting_word) > 0:
		prompt = prompt + ('\n\n%s' % (starting_word))
	if howto:
...
if __name__ == '__main__':
	...
	for i in range(args.num_stories):
		...
		if args.topic == '':
			...
			stories = make_topical_stories(args.topic, rep_pen=args.rep_pen, temp=args.temp, resp_length=args.resp_length, howto=HOWTO, starting_word=STARTING_WORD)
